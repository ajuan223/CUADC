...
`REGISTRY.md` SHALL 注册以下新增或重命名的公共能力：`partial_write_mission`、`download_mission`、`GpsDropPoint`、`MAVLinkConnection.flightmode`、`MissionContext.set_drop_point`、`MissionContext.update_mission_seq`。

#### Scenario: 新能力已注册
- **WHEN** `REGISTRY.md` 被读取
- **THEN** 表格中包含 `partial_write_mission`、`download_mission`、`GpsDropPoint` 的条目
