...
### Requirement: Strike full-chain validation SHALL cover the complete autonomous mission chain with runnable guarded validation
The project MUST provide runnable validation for the full SITL mission chain from startup through mission completion, rather than leaving the end-to-end scenarios as unconditional placeholders.

#### Scenario: Normal strike path completes end-to-end
- **WHEN** the validated SITL stack is available and the normal full-chain validation runs for `sitl_default`
...
- **AND** it MUST verify that the run reaches its successful completion signal without transport-layer failure

#### Scenario: Missing SITL prerequisites skip with an actionable reason
- **WHEN** the required local SITL stack prerequisites are unavailable in a development environment
- **THEN** the full-chain validation MUST skip rather than fail ambiguously
- **AND** it MUST report the prerequisite that prevented the run
