...
The system MUST allow an operator to load a historical `flight_log` into Field Editor and replay the recorded aircraft trajectory on the map without requiring a live telemetry connection.

#### Scenario: Operator loads a historical flight log
- **WHEN** the operator selects a valid historical `flight_log` for a completed mission
...
- **WHEN** the replay timeline is positioned at a specific recorded sample
...
The system MUST provide replay controls sufficient for post-flight inspection, including play, pause, progress seeking, playback speed selection, and fitting the map view to the replayed trajectory.

#### Scenario: Operator plays and pauses replay
- **WHEN** the operator starts or pauses replay
- **THEN** Field Editor MUST advance or stop the replay cursor accordingly

#### Scenario: Operator seeks within replay timeline
- **WHEN** the operator drags the replay progress control to a different point in the mission
...
- **WHEN** the operator selects a supported playback speed
...
- **WHEN** the operator requests a trajectory fit action
...
The system MUST display replay data together with planning-time field geometry so operators can compare actual flight behavior and drop outcomes against the configured field profile.

#### Scenario: Replay shows planning geometry
- **WHEN** a replay is opened for a field profile with boundary, runway, scan preview, or attack-run preview
...
- **WHEN** the loaded replay data contains planned drop context and an actual drop result
...
- **WHEN** the loaded replay data does not contain a confirmed actual drop point
...
- **WHEN** the operator uses the new replay capability
