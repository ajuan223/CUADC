...
### Requirement: Landing configuration with derived approach preview
The editor SHALL allow users to place and edit the landing touchdown point and landing parameters. The editor SHALL render a touchdown marker, a heading line, and a derived landing approach preview using the same parameter semantics as the backend landing geometry.

#### Scenario: Place or drag touchdown point
- **WHEN** the user places or drags the touchdown point on the map
- **THEN** the touchdown marker position updates
- **AND** `landing.touchdown_point.lat/lon` in editor state is updated

#### Scenario: Update landing heading preview
- **WHEN** the user changes `landing.heading_deg`
- **THEN** the editor updates the touchdown heading line on the map
- **AND** any derived landing previews are recomputed

#### Scenario: Show derived landing approach preview
- **WHEN** touchdown point, `heading_deg`, `glide_slope_deg`, and `approach_alt_m` together define a valid derived approach
...
- **WHEN** `approach_alt_m` is not above touchdown altitude, or the derived approach distance is below 200m, or the derived approach point falls outside the geofence
...
#### Scenario: Touchdown point outside boundary
...
### Requirement: Scan parameter editing with boustrophedon preview
The editor SHALL allow users to edit scan parameters and preview the generated boustrophedon scan path on the map. The preview SHALL follow the same parameter semantics as the backend scan generation logic.

#### Scenario: Render scan preview from valid inputs
- **WHEN** the boundary polygon and `scan.altitude_m`, `scan.spacing_m`, and `scan.heading_deg` are all defined with valid values
- **THEN** the editor renders the generated scan path as a polyline on the map

#### Scenario: Update scan preview on parameter change
- **WHEN** the user changes scan spacing or heading
- **THEN** the scan preview is recomputed and redrawn immediately
