...
async def upload_mission(conn: MAVLinkConnection, items: list[Any]) -> None:
    ...
    # SITL may re-request a sequence if it rejected an item, so we must send
    # the item matching the requested seq, not just the loop counter.
    sent_count = 0
    while sent_count < count:
        try:
            ...
            # Track progress: if SITL is requesting beyond what we've counted, advance
            if req_seq >= sent_count:
...
async def upload_attack_mission(
    ...
    attack_alt = field_profile.scan_waypoints.altitude_m
    landing_items = generate_landing_sequence(field_profile, conn.mav, start_seq=0)
    # Landing items will be re-sequenced by build_attack_run_mission
